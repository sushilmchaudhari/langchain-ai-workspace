...
# Define a Pydantic model for the structured output - Schema for the customer support response

class CustomerReview(BaseModel):
    summary: str = Field(..., description="A brief summary of the customer review in 50-70 words")
    sentiment: Literal["positive", "negative", "neutral"] = Field(..., description="The overall sentiment of the review either positive, negative or neutral based on the content of the review")
    pros: Optional[list[str]] = Field(None, description="A list of top 3 positive aspects mentioned in the review if available and explicitely mentioned as Pros:, otherwise empty list") 
    # cons: Optional[list[str]] = Field(None, description="A list of top 3 negative aspects mentioned in the review if available and explicitely mentioned as Cons:, otherwise empty list")

    cons: Annotated[Optional[list[str]], "A list of top 3 negative aspects mentioned in the review if available and explicitely mentioned as Cons:, otherwise empty list"] = None
